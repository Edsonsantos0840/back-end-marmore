...
//     res
//       .status(500)
//       .json({ error: "Erro ao buscar produtos, tente novamente mais tarde." });
//   }
// };
export const getProducts = async (req: Request, res: Response): Promise<void> => {
  try {
    const limit = parseInt(req.query.limit as string) || 10; // padrão 10 itens por página
    const page = parseInt(req.query.page as string) || 1;
    const skip = (page - 1) * limit;

    // Busca paginada + total em paralelo
...
export const getProductsByCategory = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const [products, total] = await Promise.all([
      Product.find({ category: category.toLowerCase() })
        .populate("comments")
        .populate("likes")
        .limit(limit)
        .skip(skip),
      Product.countDocuments({ category: category.toLowerCase() })
    ]);

    if (!products || products.length === 0) {
      res.status(404).json({
        error: `Nenhum produto encontrado na categoria "${category}".`
      });
      return;
    }
...
export const getProductById = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar produto, tente novamente mais tarde." });
  }
};
